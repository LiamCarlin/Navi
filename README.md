...
- **Voice control** — click the sparkle in the ⌘Space bar (or the menu bar) and an island drops out of the notch. Talk naturally and keep talking: Apple's on-device recognizer streams your words, Navi splits them into instructions, and Jev decides *the moment each one is complete* — "open up the notes app" launches Notes while you're still saying "and make hello the title", which then runs in Notes. Apps open instantly, tasks run on the Jev-first driver, questions stream from Claude into the island; say "stop", "undo", "yes"/"no" or "stop listening". Audio never leaves your Mac.
