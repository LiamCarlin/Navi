        ...
        <Reveal>
          <div className="flex flex-col items-start justify-between gap-6 md:flex-row md:items-end">
            <div className="max-w-xl">
              ...
              <p className="lede mt-4">No API keys, no model pickers. Start free; upgrade when you hit the wall.</p>
            </div>
            <div role="group" aria-label="Billing period" className="inline-flex rounded-full border border-line bg-glass p-1 text-sm">
        ...
        <div className="mt-10 grid grid-cols-1 gap-4 md:grid-cols-3 md:gap-6">
          {plans.map((p, i) => (
            <Reveal key={p.name} delay={i * 0.06}>
              <article className={`card flex h-full flex-col p-6 ${p.featured ? "border-accent/50 bg-accent-soft/40" : ""}`}>
                <div className="flex items-center justify-between">
                  <h3 className="text-lg font-semibold tracking-tight">{p.name}</h3>
                  {p.featured && <span className="rounded-full bg-accent px-2 py-0.5 text-[11px] font-medium text-bg">Popular</span>}
                </div>
                <p className="mt-1 text-sm text-fg-muted">{p.blurb}</p>
                <div className="mt-6 flex items-baseline gap-1">
                  <span className="text-4xl font-semibold tracking-tight tabular-nums">${yearly ? p.yearly : p.monthly}</span>
                ...
                <a href="#waitlist" className={`mt-8 !h-10 ${p.featured ? "btn-primary" : "btn-secondary"}`}>
                  Join the waitlist
                </a>
              </article>
            </Reveal>
...
function Toggle({ active, onClick, children }: { active: boolean; onClick: () => void; children: React.ReactNode }) {
  return (
    <button
      ...
      aria-pressed={active}
      onClick={onClick}
      className={`rounded-full px-4 py-1.5 transition-colors duration-200 ${active ? "bg-fg text-bg" : "text-fg-muted hover:text-fg"}`}
    >
      {children}
    </button>
  );
